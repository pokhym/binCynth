...
ADDRESS = 0x0000000
# size of memory
MEMORY_SIZE = 1024 * 1024 * 1024 # 2MB
mu = uni.Uc(UC_ARCH_X86, uni.UC_MODE_64)
mu.mem_map(ADDRESS, MEMORY_SIZE) #, uni.UC_PROT_ALL)
...
def load_binary(bin_path : str):
    ...
    for phdr in phdrs:
        ...
        ba = bytearray()
        for a in phdr.content:
            ba.append(a)
        # hex_str = "\\x" + "\\x".join("{:02x}".format(x) for x in ba)
        b = bytes(ba)
        # print(type(hex_str), type(b"\x00\x01"), type(ba))
        mu.mem_write(vaddr, b)
...
def execute_binary():
    ...
    try:
        ...
        mu.emu_start(BINARY.get_symbol('main').value, BINARY.get_symbol('main').value + 0x1000)
        a = 0
